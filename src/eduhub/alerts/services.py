...
from .slack_client import SlackConfigurationError, get_slack_client
...
class AlertDispatchService:
    ...
    def __init__(self):
        ...
        self.alert_ttl = (
            int(os.getenv("ALERT_RETENTION_DAYS", "30")) * 24 * 3600
        )  # seconds
        self.max_broadcast_timeout_ms = int(
            os.getenv("ALERT_BROADCAST_TIMEOUT_MS", "50")
        )
    ...
    async def dispatch_alert(self, alert: Alert) -> AlertResponse:
        ...
        try:
            ...
            # Determine status
            successful_channels = [
                channel for channel, success in delivery_results.items() if success
            ]
            failed_channels = [
                channel for channel, success in delivery_results.items() if not success
            ]

            if not successful_channels:
                self._metrics["failed_deliveries"] += 1
                raise AlertDeliveryError(
                    f"Alert delivery failed on all channels: {failed_channels}"
                )

            if failed_channels:
                logger.warning(
                    f"Alert {alert.id} partially delivered. Failed: {failed_channels}"
                )
                status = "partial"
            else:
                status = "delivered"
            ...
            logger.info(
                f"Alert {alert.id} dispatched in {dispatch_time:.2f}ms to {successful_channels}"
            )
    ...
    async def create_and_dispatch_alert(
        self, request: AlertRequest, source: str = "api"
    ) -> AlertResponse:
        """
    ...
    async def _dispatch_to_channels(self, alert: Alert) -> dict[str, bool]:
        ...
        if tasks:
            try:
                ...
                for channel in alert.channels:
                    if channel in [AlertChannel.WEBSOCKET, AlertChannel.SLACK]:
                        ...
                        if isinstance(result, Exception):
                            logger.error(
                                f"Channel {channel.value} dispatch failed: {result}"
                            )
                            delivery_results[channel.value] = False
                        else:
                ...
                logger.error(
                    f"Alert dispatch timeout after {self.max_broadcast_timeout_ms}ms"
                )
    ...
    async def _is_duplicate_alert(self, alert: Alert) -> bool:
        ...
        content_hash = hash(
            f"{alert.title}:{alert.message}:{alert.category}:{alert.user_id}"
        )
    ...
    async def _mark_for_deduplication(self, alert: Alert) -> None:
        ...
        try:
            await self.redis_client.setex(
                dedup_key, self.deduplication_window_seconds, str(alert.id)
            )
        except Exception as e:
    ...
    async def _persist_alert(self, alert: Alert) -> None:
        ...
        try:
            ...
            await self.redis_client.hset(self.alerts_key, str(alert.id), alert_data)

            # Set expiration on the alert
            if alert.expires_at:
                expire_seconds = int(
                    (alert.expires_at - datetime.utcnow()).total_seconds()
                )
                if expire_seconds > 0:
                    await self.redis_client.expire(
                        f"{self.alerts_key}:{alert.id}", expire_seconds
                    )
            else:
                # Default TTL
                await self.redis_client.expire(
                    f"{self.alerts_key}:{alert.id}", self.alert_ttl
                )
    ...
    async def _update_metrics(self, delivery_results: dict[str, bool]) -> None:
        ...
        try:
            ...
            metrics_data = {"timestamp": datetime.utcnow().isoformat(), **self._metrics}

            await self.redis_client.hset(
                self.metrics_key, "current", json.dumps(metrics_data)
            )
    ...
    def get_metrics(self) -> AlertMetrics:
        ...
        return AlertMetrics(
            ...
            active_subscriptions=websocket_manager.get_metrics()[
                "active_subscriptions"
            ],
            average_broadcast_latency_ms=float(
                self.max_broadcast_timeout_ms
            ),  # Approximation
    ...
    async def test_channels(self) -> dict[str, Any]:
        ...
        if self.redis_client:
            try:
                await self.redis_client.ping()
                results["redis"] = {"success": True, "connected": True}
            except Exception as e:
...
AlertService = AlertDispatchService
