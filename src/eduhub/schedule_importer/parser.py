...
class ScheduleParser:
    ...
    async def parse_file(
    ...
    ) -> tuple[list[ScheduleRow], list[ValidationError]]:
        """
        ...
        with tempfile.NamedTemporaryFile(
            delete=False, suffix=self._get_file_suffix(file.filename)
        ) as tmp_file:
            try:
    ...
    def _read_dataframe(
        self, filepath: str, original_filename: Optional[str]
    ) -> pd.DataFrame:
        """Read file into pandas DataFrame."""
        try:
            if original_filename and original_filename.lower().endswith(
                (".xlsx", ".xls")
            ):
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                for field, value in row_data.items():
                    if field in self.REQUIRED_COLUMNS and (
                ...
                self.validation_errors.append(
                    ValidationError(
                        row_number=row_num, field=None, message=str(e), value=None
                    )
                )
            except Exception as e:
                # Unexpected error
    ...
    def _safe_int(self, value) -> Optional[int]:
        ...
        try:
            return int(
                float(value)
            )  # Handle cases where Excel stores integers as floats
