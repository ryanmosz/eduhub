...
class TestScheduleImporterPloneIntegration:
    ...
    async def test_create_single_event_no_uid_in_response(self, mock_plone_client, test_user):
    ...
    async def test_rollback_created_content_success(self, mock_plone_client, test_user):
        ...
        call_args = [call[0][0] for call in mock_plone_client.delete_content.call_args_list]
        assert "events/event-123" in call_args
        assert "events/event-456" in call_args
        assert "events/event-789" in call_args

    async def test_rollback_created_content_partial_failure(self, mock_plone_client, test_user):
    ...
    async def test_rollback_created_content_exception(self, mock_plone_client, test_user):
    ...
    async def test_create_plone_content_success(self, mock_plone_client, test_user, sample_schedule_rows):
        ...
        result_uids = await service._create_plone_content(sample_schedule_rows, test_user)
    ...
    async def test_create_plone_content_with_rollback(self, mock_plone_client, test_user, sample_schedule_rows):
        ...
        mock_plone_client.create_content.side_effect = mock_responses + [PloneAPIError("Creation failed")]
        mock_plone_client.delete_content.return_value = True
...
class TestScheduleImporterEndToEnd:
    ...
    @patch("eduhub.schedule_importer.services.ScheduleImportService._create_single_event")
    async def test_rollback_on_second_row_failure(self, mock_create_event, sample_schedule_rows):
